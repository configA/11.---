...
void inline PWM_Change_3(int dt)//输入范围0-180
{
	Pwm_angle = 2.3*dt + 180;
	dt = (int)Pwm_angle;
	_stm0al=dt&0x00ff;//PTMn CCRA 高字节寄存器 bit 1~bit 0    
	_stm0ah=dt>>8;    //PTMn CCRA 高字节寄存器 bit 1~bit 0
}
...
//	_pds04 = 1;  	   //PD2配置为PTP2输出  左上臂
	...
	// //	STM0	PWM Output	 CCRP 是 8 位宽度， 与计数器的高 8 位比较； 而 CCRA 是 16 位的， 与计数器的所有位比较。
	_stm0c0=0b00110100;	//  Fsys/64;	
	_stm0c1=0b10101000;	//  
	//  CCRP=Fsys/(64*输出频率)	625--> PWM 频率200Hz 频率100Hz以上看起来无闪烁   舵机2500  
	_stm0rp=5000&0xff;  //PTMn CCRP 字节寄存器 bit 7~bit 0   配置周期 
	_st0on=1;
...
//	PTM0	PWM Output
	...
	//  CCRP=Fsys/(64*输出频率)	625--> PWM 频率200Hz 频率100Hz以上看起来无闪烁   舵机2500  
	_stm0rp=5000&0xff;  //PTMn CCRP 字节寄存器 bit 7~bit 0   配置周期 
	// _stm0rph=5000>>8;    //PTMn CCRP 高字节寄存器 bit 7~bit 0
	_st0on=1;
